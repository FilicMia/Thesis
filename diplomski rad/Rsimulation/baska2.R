...
#Let T = t+tau - universal clock time at point of the receive of the signal 
#= true receive clock time + clock bias
#T_s = t_s + tau_s - Universal clock time at point of the transmition of the signal 
#= true satellite time + satellite clock bias(due to the satellite)
...
#(A^T)A 
#exists.  For  example, m≥4is  a  necessary  (but  not sufficient)  condition.    Problems  can  exist  if,
#for  example,  a  pair  of  satellites  lie  in  the  same 
#line  of  sight,  or  if  the  satellites  are  all  in  the  same  orbital  plane.  
#In  almost  all  practical 
#situations, m≥5is  sufficient.    
#Alternatively,  one  parameter  could  be  left  unestimated  (e.g.,                                                                    the height could be fixed to sea-level for a boat)
#
