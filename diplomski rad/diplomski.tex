...
$$
Opisanim perturbacijama, dobivamo sljedeću izvedbu algoritma \ref{code:iterLSMW}.
...
Rješenja lineariziranog i nelineariziranog sustava nisu u potpunosti jednaka, ali su u ovo slučaju dovoljno bliska jer su jednadžbe sustava skoro pa linearne \cite{singer07}.%slide 16
...
Ideja je da, budući da zbog pogreške u mjerenjima sustav \ref{eq:sustav} nema uvijek rješenje, tj. 
...
Dakle, sada rješavamo sustav
\begin{align}\label{eq:sustavQR}
	\mathbf{R}^T\mathbf{R}\mathbf{x} = \mathbf{R}^T\mathbf{Q}^T\mathbf{b} = \mathbf{A}^T\mathbf{b}.
\end{align}
