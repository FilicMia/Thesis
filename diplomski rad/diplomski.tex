...
\end{align}
gdje je $ p_i(\textbf{x}) = - \sqrt{(x-x_i)^{2}+(y-y_i)^{2}+(z-z_i)^{2}}v_i + v_i^2$.
Označimo 
$$p'(\mathbf{x}) := (p_1(\textbf{x}),p_2(\textbf{x}),p_3(\textbf{x}),p_4(\textbf{x}))$$
...
$$ A:=  
\begin{bmatrix}
(d_1-d)^2 & (x-x_1)^{2} & (y-y_1)^{2} & (z-z_1)^{2} \notag \\
(d_2-d)^2 & (x-x_2)^{2} & (y-y_2)^{2} & (z-z_2)^{2} \notag \\
(d_3-d)^2 & (x-x_3)^{2} & (y-y_3)^{2} & (z-z_3)^{2} \notag \\
(d_4-d)^2 & (x-x_4)^{2} & (y-y_4)^{2} & (z-z_4)^{2} \notag \\
\end{bmatrix}
$$
$$ I':=  diag(-1,1,1,1) $$
$$  \mathbf{1} = diag[ 1, 1, 1, 1]$$
$$ D' = \text{dijagonala matrica}(A I' A^T)  = diag(- (d_i-d)^2 + (x-x_i)^{2} + (y-y_i)^{2} + (z-z_i)^{2})$$
imamo 
\begin{align*}
\hat{\mathbf{x}} = \text{arg min}_\mathbf{x} \mathbf{p'}(\mathbf{x})^T\mathbf{p'}(\mathbf{x}) = \text{arg min}_\mathbf{x} (D')^TD'
\end{align*}
Ovim perturbacijama, dobivamo sljedeću izvedbu algoritma \ref{code:iterLSMW}.
...
unutar = append(R,S)
A = matrix(unutar,4,4) # [d,x,y,z]

while(norm(t(error)) > 2){
	preRhoVector = t(apply(A, 1, function(x) (x-x_0))) #x-x_0
	D = t(t(apply(preRhoVector,1, function(x) norm(t(x),'F')))) # D' = diagonala(A^TI'A)
	DD = D
	DD[1] = -D[1]
...
Tom izvedbom dobivamo numerički nestabilan sustav kojidivergira.

\subsection{Odabran pristup}
%DODATI U LITERATURU str 366.
%https://web.math.pmf.unizg.hr/~singer/num_alg/num_anal.pdf

\subsubsection{Numerička metoda koja dovodi do poboljšanja}
