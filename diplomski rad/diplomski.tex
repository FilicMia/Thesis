...
Osnovni algoritam čini algoritam najmanjih kvadrata (Stranica \pageref{code:iterLSMW}, algoritam \ref{code:iterLSM} i \ref{code:iterLSMW}).
\\Za dobivene psudoudaljenosti (vidi \ref{sssec:rinex}), i 
...
Prilikom izvedbe korišten je programski jezik $R$ \cite{ref:24} i R-sučelje \textit{RStudio}
\section{Programski jezik R}
$R$ je programski jezik za statističku i drugu matematičku obradbu putem računala koji ima snažnu grafičku potporu.
$R$ omogućava izvedbu statističke analize, modeliranje i simulacija.
 Između ostalog, podržani su postupci zasnovani na linearnoj algebri, analizi i prognozi ponašanja vremenskih nizova i grafičkom predočavanju \cite{ref:34,ref:22}. 
 \\
 
 \textit{R} je dostupan za većinu platformi (Windows, Linux, Mac) i instalacija je poprilično jednostavna.
 Potrebno je preuzeti potrebne datoteke s web-stranice R-site i instalirati program u skladu s njima.
 Instalirani program nudi R-sučelje (R-GUI) u kojemu se preko naredbene linije zadaju naredbe i pokreću skripte i dobivaju numerički i grafički rezultati.
 Postoji i više neslužbenih R-sučelja. Jedan od poznatijih je \textit{R-Studio} \cite{RStudio}.
 Ovdje se također komunikacija ostvaruje preko naredbi u konzoli, ali je RStudio opremljen znantno bogatijom grafičkom okolinom (radni prostor, povijest, instalacija paketa, pomoć i sl.).
...
Ideja je da, budući da zbog pogreške u mjerenjima ili linearizacije sustav \ref{eq:sustav} nema uvijek rješenje, tj. 
...
Dakle, $\mathbf{\hat{x}}$ je rješenje problema najmanjih kvadrata ako i samo ako
vrijedi $\mathbf{A}\mathbf{\hat{x}} = \mathbf{\hat{b}}$, tj.
$\mathbf{\hat{x}}:= \mathbf{(A^TA)^{-1}A^T}\mathbf{b}$, odnosno $\mathbf{\hat{x}}$
je \textbf{rješenje problema normalnih jednadžbi} \begin{align}\label{eq:sustavProj}
A^TA\mathbf{x} = A^Tb.
\end{align}
\end{proof}
...
Zanimljivo je da 
ukoliko znamo jedno rješenje sustava \ref{eq:sustavProj}, lako je pronaći i sva ostala.\\
Naime, uz $\mathbf{A}\mathbf{x} - \mathbf{b} = r$, $\hat{x} \in \R^m$ proizvoljan i
\begin{align}
\hat{r}  &=  \mathbf{A}\mathbf{\hat{x}} - \mathbf{b} \notag \\
&= r + A\mathbf{x} - A\mathbf{\hat{x}} \notag \\
&= r - A(\mathbf{\hat{x}} - \mathbf{x}) \notag
\end{align}
imamo $\mathbf{\hat{x}} \in S$ ako i samo ako $\hat{r} = r$ pa  $ A(\mathbf{\hat{x}} - \mathbf{x}) = 0$, tj. $\mathbf{\hat{x}} - \mathbf{x} \in \mathcal{N}(A)$.\\

Nadalje, ukoliko vrijedi nešto od sljedećega:
\begin{itemize}
...
%PROVJERI OVAJ DOLE ZAKOMENTIRANI - NIJE DOBAR JER JE U KORAKU K POTREBNO ODRETIDI
%DElta_cd_T_{k+1}-DElta_cd_T_{k} <- a k+1 tek treba izračunati
...
%\end{align*}
%Analogno kao zu prošlom potpoglavlju, dobivamo:
%\begin{align*}
...
\subsection{Poboljšan pristup: uvođenje težina (WLSM)}\label{sec:algTezine}%dipl4.R
